...
def writer(folder: Path) -> int | None:
    ...
    if child is not None and child.poll() is not None:
        if child.returncode:
            (folder / 'runner-status.json').write_text(json.dumps({
                'pid': child.pid, 'state': 'failed', 'reason': '运行异常，请查看实验目录中的 runner.log'}))
...
def status(folder: Path, world: dict | None = None) -> dict:
    ...
    reason = ''
    if pid:
        state = 'stopping' if (read(folder, 'runner-control.json').get('stopPid') == pid or
                              (saved.get('pid') == pid and saved.get('state') == 'stopping')) else 'running'
    ...
    current = re.search(r"RULES_VERSION = '([^']+)'", (ROOT / 'frontend/src/sim/world.ts').read_text())[1]
    compatible = world.get('rulesVersion') == current
    if not compatible and state == 'paused':
        reason = '此实验使用旧版规则，请开始新实验'
    settings = read(folder, 'runner-settings.json')
    return {'state': state, 'reason': reason, 'canResume': state in ('paused', 'failed') and compatible,
            'concurrency': settings.get('concurrency', 6), 'mode': settings.get('mode', 'llm')}
...
def command(folder: Path, mode: str, concurrency: int) -> list[str]:
    ...
    return [node, '--import', 'tsx', str(ROOT / 'scripts/simulate.ts'), '--out', str(folder),
            '--mode', mode, '--concurrency', str(concurrency),
            '--api', os.getenv('SIMULATION_API_URL', 'http://127.0.0.1:8000')]
...
def launch(folder: Path, mode: str, concurrency: int) -> dict:
    ...
    with (folder / 'runner.log').open('ab') as log:
        child = subprocess.Popen(command(folder, mode, concurrency) + ['--resume'], cwd=ROOT,
    ...
    while time.monotonic() < deadline:
        ...
        if read(folder, 'runner-status.json').get('pid') == child.pid and writer(folder) == child.pid:
            return status(folder)
        time.sleep(0.05)
...
def resume(folder: Path) -> dict:
    with _lock:
        ...
        if not info['canResume']:
            raise HTTPException(409, info['reason'] or '实验运行中、保存中或已完成，不能继续')
        mode = read(folder, 'runner-settings.json').get('mode') or read(folder, 'summary.json').get('mode', 'llm')
...
def create(artifacts: Path, config: dict, mode: str, concurrency: int) -> dict:
    with _lock:
        ...
        try:
            result = subprocess.run(command(folder, mode, concurrency) +
                                    ['--config', str(folder / 'initial-config.json'), '--initialize-only'],
                                    cwd=ROOT, capture_output=True, timeout=20)
        except subprocess.TimeoutExpired:
            raise HTTPException(503, '初始化超时，请检查后台环境')
        (folder / 'runner.log').write_bytes(result.stdout + result.stderr)
        if result.returncode:
            raise HTTPException(422, '实验配置无效或初始化失败，请检查参数及 runner.log')
