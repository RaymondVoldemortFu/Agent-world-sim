...
PROMPT_VERSION = 'world-agent-v1.11'
...
self.social 显示你的 loneliness 孤单值、capacity 上限、lastSpokeDay 最近有效说话日和 depressed 抑郁状态。孤单条上限 round(100-60*外向性)，外向性0..1，越外向越容易满。连续两天没向人说话，从第二天日末起每天孤单+20，最多到上限；对至少一名活着的听众成功使用 chat 或 shout，每次孤单-20并重新计时。只听别人说话不算自己说过话，对空地自言自语也不算。条满时进入抑郁，此后每个日末额外扣10生命，孤单未清零就会持续，清零后才解除。其他休养回血规则仍照常结算。
普通物品每单位重1、工具重2，背包容量由physicalRules.inventoryCapacity给出（旧记录未提供时为12）。棚屋为同格居民提供休养收益。
地面物品可拿取，土地和建筑不具有系统强制的产权；你可形成自己的理解。
幼年拥有相同的每日AP，只可move/eat/take/drop/place/give/feed/chat/shout/survey/wait，依赖食物和照料。
