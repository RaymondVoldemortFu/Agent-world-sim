...
class ThinkingTransportTests(unittest.IsolatedAsyncioTestCase):
    async def test_only_deep_reflection_enables_thinking(self):
        post = AsyncMock(return_value=SimpleNamespace(status_code=200, json=lambda: {
            'choices': [{'message': {'content': '{"intent":"test"}'}}], 'usage': {}}))
        async def decide(req, model, send):
            for purpose in ['decision', 'compression', 'deep_reflection']:
                await send([{'role':'user','content':'JSON'}], 8192, purpose)
            return {'ok':True}
        with patch.dict(os.environ, {'DEEPSEEK_API_KEY':'test'}), patch.object(ce, 'decide', decide), \
            ...
            await decision(DecisionRequest(runId='test', decisionId='test', context={'protocol':'context-1'}))
        self.assertEqual([c.kwargs['json']['thinking']['type'] for c in post.call_args_list], ['disabled','disabled','enabled'])
...
class ReflectionContextTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_reflection_keeps_prefix_and_is_idempotent(self):
        ...
        prefix = ce.fixed(o)
        o.update(day=10, seq=2, deepReflection=True, contextHead=first['contextTrace']['turnId'])
        self.assertEqual(prefix, ce.fixed(o))
        calls=[]
        async def provider(messages, limit, purpose):
            calls.append((messages,limit,purpose))
            return {'status':200,'purpose':purpose,'content':json.dumps({'intent':'协作','reflection':{'summary':'近期资源紧缺','plan':'与邻居合作生产工具'}}),'usage':{}}
        result = await ce.decide(self.request(o), 'test', provider)
        self.assertEqual(calls[-1][1:], (min(8192, ce.context_window(o) // 3),'deep_reflection'))
