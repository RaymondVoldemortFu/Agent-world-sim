...
RUN = 'continuous-1789196221885-abbea8'
OUT = Path('reports/latest-manor/data.json')
...
def main():
    ...
    plans=[]; transfers=[]; errors=[]; alarms=[]; letters=[]; modelrows=[]; operation_tasks={}
    ...
    with s.transaction() as q:
        ...
        head = q.fetchone(); final = s.decode(head['state']); cutoff = head['sim_time']
        ...
        while last < head['seq']:
            ...
            if not rows: break
            for row in rows:
                e=s.decode(row['payload']); typ=e['type']; actor=e.get('actor'); day=int(e['time']//DAY)+1
                if row['seq'] != expected: gaps.append([expected,row['seq']])
                expected=row['seq']+1
                patch=e['patch']
                if typ=='estate': operation_tasks[e['seq']]=arrays['agents'].get(actor,{}).get('action',{}).get('operation')
                if typ in ('estate','withdraw','deposit','harvest'):
                    changes=[]
                    for store in patch.get('stores',[]):
                        old=arrays['stores'].get(store['id'],{}).get('grain',0)
                        delta=store['grain']-old
                        if abs(delta)>1e-7:changes.append({'store':store['id'],'delta':delta,'after':store['grain']})
                    if changes:transfers.append({'seq':e['seq'],'time':e['time'],'actor':actor,'text':e['text'],'type':typ,'changes':changes})
                if typ=='plan':
                    a=next((a for a in patch.get('agents',[]) if a['id']==actor),{})
                    plans.append({'seq':e['seq'],'time':e['time'],'actor':actor,'text':e['text'], **{k:a.get(k) for k in ('x','y','grain','food','intent','task','routine','blocked')}})
                if typ=='door_noise':alarms.append({k:e.get(k) for k in ('seq','time','actor','text','listeners')})
                if typ in ('letter_delivery','royal_return','royal_departure','routine_interrupted','access_expired'):
                    letters.append({k:e.get(k) for k in ('seq','time','actor','text','listeners','type')})
                if typ=='plan_rejected' or (typ in ('estate','think_finished') and '失败' in e.get('text','')):
                    errors.append({k:e.get(k) for k in ('seq','time','actor','text','type')})

                for k in arrays:
                    for a in patch.get(k,[]): arrays[k][a['id']]=a
                merge_meta(w,patch); w['seq']=e['seq']; w['time']=e['time']
                ...
                if typ in ('death','season','royal','control','witness','royal_return','royal_departure','recovery'):
        ...
        q.execute('SELECT request_id,actor,payload FROM continuous_thoughts WHERE run_id=%s',(RUN,))
        for row in q.fetchall():
            t=s.decode(row['payload']);u=t.get('usage',{});
            modelrows.append({'id':row['request_id'],'actor':row['actor'],'elapsedMs':t.get('elapsedMs'),'usage':u,'error':t.get('error'),'attempts':[{k:a.get(k) for k in ('stage','errorCode','finishReason','elapsedMs','repairs')} for a in t.get('attempts',[])]})
            for k,v in u.items():
        ...
        out['operations'] = [{**{k:e.get(k) for k in ['seq','time','actor','text','type','listeners']},'task':operation_tasks.get(e['seq'])} for e in [s.decode(r['payload']) for r in q.fetchall()]]
        q.execute('SELECT actor,payload FROM continuous_contexts WHERE run_id=%s AND actor IN (1,2,3,7,29,32)', (RUN,))
        out['lastHungerContexts'] = {r['actor']:s.decode(r['payload'])['tail'][-4:] for r in q.fetchall()}
    out.update(plans=plans,transfers=transfers,errors=errors,alarms=alarms,letterEvents=letters,modelRows=modelrows,metrics=metrics,health=health,population=population,deaths=deaths,important=important,speeches=speeches,
